		...
		if (kx > 0) {
			...
			for (int y = 0; y < H; ++y) {
				...
				for (int x = 0; x < W; ++x) {
					...
					for (int k = -kx; k <= kx; ++k) {
						int xx = clampf((float)(x + k), 0, (float)(W - 1));
						const CRT_FPixel& p = row[xx];
						s.r += p.r; s.g += p.g; s.b += p.b; s.a += p.a;
					}
					out->r = s.r * inv; out->g = s.g * inv; out->b = s.b * inv; out->a = s.a * inv; ++out;
				}
			}
		...
		if (ky > 0) {
			...
			for (int x = 0; x < W; ++x) {
				for (int y = 0; y < H; ++y) {
					CRT_FPixel s = { 0,0,0,0 };
					for (int k = -ky; k <= ky; ++k) {
						int yy = clampf((float)(y + k), 0, (float)(H - 1));
...
void CRT_RenderImage(CRT_FPixel* buf, const CRT_Settings& s) {
	...
	if (haveGlow) {
		...
		for (size_t i = 0; i < N; ++i) {	/* glow saturation */
	...
	if (s.auto_exposure) {
		...
		if (mean > 1e-4f) {
			...
			for (size_t i = 0; i < N; ++i) { buf[i].r *= f; buf[i].g *= f; buf[i].b *= f; }
		}
	}
